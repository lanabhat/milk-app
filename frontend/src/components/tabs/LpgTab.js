...
export default function LpgTab({ showToast }) {
  ...
  useEffect(() => { fetchLpg(); fetchLpgTrends(); }, [fetchLpg, fetchLpgTrends]);
  useEffect(() => { if (lpg) fetchLpgTrends(); }, [lpg, fetchLpgTrends]);
  ...
  const handleLpgSaveConfig = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/lpg/config/`, {
        method: 'POST', headers,
        body: JSON.stringify({ waiting_days: parseInt(lpgWaiting || 0), total_cylinders: total, filled_cylinders: filled, empty_cylinders: empty }),
      });
      if (res.ok) { showToast('✓ Settings updated'); setSettingsModalOpen(false); fetchLpg(); }
      else {
        const err = await res.json().catch(() => ({}));
        showToast(err.non_field_errors?.[0] || err.detail || 'Error saving settings', 'error');
      }
    } catch { showToast('Network error', 'error'); }
  };

  const handleLpgBook = async () => {
    setLpgSaving(true);
    const headers = getAuthHeaders();
    if (!headers) return;
    try {
      const res = await fetch(`${API}/api/lpg/book/`, {
        method: 'POST', headers,
        body: JSON.stringify({ booking_date: lpgBookDate, delivered_date: lpgDelivDate || null, price: lpgPrice ? parseFloat(lpgPrice) : null, notes: lpgNotes }),
      });
      if (res.ok) {
  ...
  const handleLpgSaveEdit = async (id) => {
    ...
    try {
      const res = await fetch(`${API}/api/lpg/${id}/booking/`, {
        ...
        body: JSON.stringify({ booking_date: lpgEditDate, delivered_date: lpgEditDeliv || null, price: lpgEditPrice ? parseFloat(lpgEditPrice) : null, notes: lpgEditNotes }),
      });
  ...
  return (
    <div style={s.section}>
      ...
      {/* Status banner */}
      {lpg && (
        <div style={{ ...s.card, textAlign: 'center', padding: '18px 16px', marginBottom: 16, borderTop: `4px solid ${lpg.can_book ? '#16a34a' : '#f59e0b'}` }}>
          {(() => {
            const eligibleDate = lpg.latest_booking
              ? (() => { const d = new Date(lpg.latest_booking.booking_date); d.setDate(d.getDate() + lpg.waiting_days); return fmtD(d.toISOString().slice(0, 10)); })()
              : null;
            return lpg.can_book ? (
              <>
                <div style={{ fontSize: 28 }}>✅</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: '#16a34a', marginTop: 4 }}>Booking Open</div>
                <div style={{ fontSize: 13, color: '#64748b', marginTop: 4 }}>{lpg.days_since} days since last booking — you can book now</div>
              </>
            ) : lpg.latest_booking ? (
              <>
                <div style={{ fontSize: 28 }}>⏳</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: '#f59e0b', marginTop: 4 }}>Wait {lpg.days_remaining} more day{lpg.days_remaining !== 1 ? 's' : ''}</div>
                <div style={{ fontSize: 13, color: '#64748b', marginTop: 4 }}>Last booked {lpg.days_since} day{lpg.days_since !== 1 ? 's' : ''} ago · eligible after {lpg.waiting_days} days</div>
                <div style={{ fontSize: 14, fontWeight: 600, color: '#92400e', marginTop: 6, backgroundColor: '#fef3c7', borderRadius: 6, padding: '4px 10px', display: 'inline-block' }}>📅 Next eligible: {eligibleDate}</div>
              </>
            ) : (
              <>
                <div style={{ fontSize: 28 }}>🔥</div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        <button style={s.addBtn} onClick={() => setBookingModalOpen(true)}>+ Record Booking</button>
        <button style={{ ...s.cancelBtn, minHeight: 44 }} onClick={() => setSettingsModalOpen(true)}>⚙ Settings</button>
      </div>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10, marginBottom: 12 }}>
            <div style={{ ...s.card, padding: 12 }}>
              ...
              <div style={{ fontSize: 12, color: '#64748b', marginTop: 4 }}>out of {lpgTrends.current_stock.total_cylinders} total</div>
            </div>
            <div style={{ ...s.card, padding: 12 }}>
              ...
              <div style={{ fontSize: 12, color: '#64748b', marginTop: 4 }}>completed cylinders</div>
            </div>
            <div style={{ ...s.card, padding: 12 }}>
                ...
                {lpgEditId === b.id ? (
                  <div>
                    <div className="form-grid" style={{ gap: 10, marginBottom: 10 }}>
                      <label>Booking Date<input type="date" value={lpgEditDate} onChange={e => setLpgEditDate(e.target.value)} style={s.input} /></label>
                      <label>Delivered Date<input type="date" value={lpgEditDeliv} onChange={e => setLpgEditDeliv(e.target.value)} style={s.input} /></label>
                      <label>Price<input type="number" step="0.01" value={lpgEditPrice} onChange={e => setLpgEditPrice(e.target.value)} style={s.input} /></label>
                      <label>Notes<input type="text" value={lpgEditNotes} onChange={e => setLpgEditNotes(e.target.value)} style={s.input} /></label>
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => handleLpgSaveEdit(b.id)} style={{ ...s.primaryBtn, fontSize: 13 }}>Save</button>
                      <button onClick={() => setLpgEditId(null)} style={{ ...s.cancelBtn, fontSize: 13 }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, flexWrap: 'wrap' }}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 4 }}>
                        {b.days_lasted != null ? `Lasted ${b.days_lasted} days` : `${b.days_since_booking} days since booking (current)`}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button onClick={() => { setLpgEditId(b.id); setLpgEditDate(b.booking_date); setLpgEditDeliv(b.delivered_date || ''); setLpgEditPrice(b.price || ''); setLpgEditNotes(b.notes || ''); }} style={{ ...s.secondaryBtn, fontSize: 12 }}>Edit</button>
                      <button onClick={() => handleLpgDelete(b.id)} style={{ ...s.secondaryBtn, fontSize: 12, color: '#dc2626', borderColor: '#fca5a5' }}>Delete</button>
                    </div>
                  </div>
                )}
              </div>
      ...
      {/* Record Booking Modal */}
      <Modal open={bookingModalOpen} onClose={() => setBookingModalOpen(false)}
        title="Record New Booking" onSave={handleLpgBook}
        saveLabel={lpgSaving ? 'Saving…' : 'Add Booking'} saving={lpgSaving}>
        <div className="form-grid">
          <label>Booking Date<input type="date" value={lpgBookDate} onChange={e => setLpgBookDate(e.target.value)} style={s.input} /></label>
          <label>Delivered Date (optional)<input type="date" value={lpgDelivDate} onChange={e => setLpgDelivDate(e.target.value)} style={s.input} /></label>
          <label>Price (optional)<input type="number" step="0.01" value={lpgPrice} onChange={e => setLpgPrice(e.target.value)} style={s.input} /></label>
          <label>Notes (optional)<input type="text" value={lpgNotes} onChange={e => setLpgNotes(e.target.value)} placeholder="e.g. Indane, refill #12" style={s.input} /></label>
        </div>
      </Modal>

      {/* Settings Modal */}
      <Modal open={settingsModalOpen} onClose={() => setSettingsModalOpen(false)}
        title="LPG Settings" onSave={handleLpgSaveConfig} saveLabel="Save Settings">
        <div className="form-grid">
          <label>Waiting Days Before Eligible<input type="number" min="1" max="90" value={lpgWaiting} onChange={e => setLpgWaiting(e.target.value)} style={s.input} /></label>
          <label>Total Cylinders<input type="number" min="0" value={lpgTotalCylinders} onChange={e => setLpgTotalCylinders(e.target.value)} style={s.input} /></label>
          <label>Filled In Stock<input type="number" min="0" value={lpgFilledCylinders} onChange={e => setLpgFilledCylinders(e.target.value)} style={s.input} /></label>
          <label>Empty In Stock<input type="number" min="0" value={lpgEmptyCylinders} onChange={e => setLpgEmptyCylinders(e.target.value)} style={s.input} /></label>
        </div>
      </Modal>
    </div>
  );
