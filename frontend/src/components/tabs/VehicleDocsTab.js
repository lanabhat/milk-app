...
export default function VehicleDocsTab({ vehicles, showToast, onSaved }) {
  ...
  const FORMS = {
    pucc: {
      fields: [
        ...
        { key: 'certificate_no', label: 'Certificate No.',  type: 'text', placeholder: 'PUCC1234' },
        { key: 'test_center',    label: 'Test Center',      type: 'text' },
        { key: 'cost',           label: 'Cost (₹)',         type: 'number' },
        { key: 'notes',          label: 'Notes',            type: 'text' },
      ],
      endpoint: 'pucc-records',
      dataKey: 'pucc',
      init: { issue_date: todayStr(), expiry_date: '', certificate_no: '', test_center: '', cost: '', notes: '' },
    },
    insurance: {
      fields: [
        { key: 'provider',      label: 'Provider',        type: 'text', placeholder: 'HDFC Ergo, Bajaj Allianz…' },
  ...
  const handleSubmit = async (e) => {
    ...
    ['cost','premium','insured_value','claimed_amount','approved_amount','max_claim_amount'].forEach(k => { if (payload[k]) payload[k] = parseFloat(payload[k]); });
    ['policy'].forEach(k => { if (payload[k]) payload[k] = parseInt(payload[k]); });
    // remove empty optional dates
    ['settlement_date','start_date','end_date'].forEach(k => { if (!payload[k]) payload[k] = null; });
    try {
      const res = await fetch(`${API}/api/${currentDef.endpoint}/`, { method: 'POST', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      if (!res.ok) throw new Error();
      showToast('✓ Saved');
      setShowForm(false); fetchData(); onSaved();
    ...
    if (docTab === 'pucc') return (
      <div key={rec.id} style={{ ...s.card, marginBottom: 8, borderLeft: `4px solid ${rec.days_until_expiry <= 0 ? '#dc2626' : rec.days_until_expiry <= 30 ? '#f59e0b' : '#16a34a'}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          {rec.cost > 0 && <div style={{ fontSize: 11, color: '#64748b' }}>Cost: ₹{fmt(rec.cost)}</div>}
      ...
      <div key={rec.id} style={{ ...s.card, marginBottom: 8, borderLeft: `4px solid ${rec.days_until_expiry <= 0 ? '#dc2626' : rec.days_until_expiry <= 30 ? '#f59e0b' : '#0369a1'}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: 12, fontWeight: 700, color: '#dc2626' }}>Premium: ₹{fmt(rec.premium)}</div>
            {rec.agent_name && <div style={{ fontSize: 11, color: '#94a3b8' }}>Agent: {rec.agent_name} {rec.agent_phone && `· ${rec.agent_phone}`}</div>}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 4 }}>
              <span style={{ fontSize: 13, fontWeight: 700 }}>₹{fmt(rec.claimed_amount)} claimed</span>
      ...
      <div key={rec.id} style={{ ...s.card, marginBottom: 8, borderLeft: `4px solid ${rec.days_until_expiry <= 0 ? '#dc2626' : rec.days_until_expiry <= 30 ? '#f59e0b' : '#7e22ce'}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 700 }}>{rec.provider}</div>
            <div style={{ fontSize: 11, color: '#64748b' }}>{fmtD(rec.start_date)} → {fmtD(rec.end_date)}</div>
            {rec.max_claim_amount && <div style={{ fontSize: 12, color: '#7e22ce', fontWeight: 700 }}>Max claim: ₹{fmt(rec.max_claim_amount)}</div>}
    ...
    <div style={s.section}>
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
        {activeVehicles.map(v => (
          <button key={v.id} onClick={() => { setSelectedVid(String(v.id)); setShowForm(false); }}
      ...
      {selectedVid && (
        <>
          {/* Doc sub-tabs */}
          <div style={{ display: 'flex', gap: 6, marginBottom: 14, borderBottom: '2px solid #e2e8f0', paddingBottom: 8 }}>
            {DOC_TABS.map(dt => (
              <button key={dt.id} onClick={() => { setDocTab(dt.id); setShowForm(false); }}
                style={{ ...s.sessionPill, ...(docTab === dt.id ? s.sessionPillActive : {}), position: 'relative' }}>
                ...
                {currentDef.fields.map(f => (
                  <div key={f.key} style={f.type === 'textarea' ? { gridColumn: '1 / -1' } : {}}>
                    ...
                    {f.type === 'select' ? (
                      ...
                      <textarea style={{ ...s.input, height: 60 }} value={form[f.key] || ''} onChange={e => setF(f.key, e.target.value)} placeholder={f.placeholder || ''} />
                    ) : (
