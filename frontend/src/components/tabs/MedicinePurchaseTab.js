  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <div style={{ fontWeight: 700, fontSize: 14 }}>🏪 Medicine Purchases <span style={{ fontSize: 12, color: 'var(--text-faint)', fontWeight: 400 }}>({purchases.length})</span></div>
        <button style={s.addBtn} onClick={() => { setForm(emptyForm()); setEditId(null); setModalOpen(true); }}>
          + Record Purchase
        </button>
      </div>
      ...
      <Modal
        open={modalOpen}
        onClose={resetForm}
        title={editId ? 'Edit Purchase' : 'Record Purchase'}
        onSave={handleSave}
        saveLabel={saving ? 'Saving…' : editId ? 'Update Purchase' : 'Save & Update Stock'}
        saving={saving}
      >
        <div className="form-grid">
          ...
          {/* Line items */}
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 8 }}>Items *</div>
            {form.items.map((item, idx) => (
              <div key={idx} style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 10, marginBottom: 8, backgroundColor: 'var(--bg2)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)' }}>Item {idx + 1}</span>
                  {form.items.length > 1 && (
                    <button style={{ ...s.iconBtn, color: '#dc2626' }} onClick={() => removeItem(idx)}>✕ Remove</button>
                  )}
                </div>
            ...
            <button style={{ ...s.cancelBtn, width: '100%' }} onClick={addItem}>+ Add Item</button>
          </div>
