...
export default function VehicleServiceTab({ vehicles, showToast, onSaved }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [rr, cr] = await Promise.all([
        fetch(`${API}/api/service-records/${selectedVid ? `?vehicle_id=${selectedVid}` : ''}`, { headers }),
  ...
  useEffect(() => { if (selectedVid) fetchAll(); }, [fetchAll, selectedVid]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!selectedVid) { showToast('Select a vehicle', 'error'); return; }
    setSaving(true);
    const headers = getAuthHeaders();
    if (!headers) return;
    const payload = {
    ...
    try {
      const res = await fetch(`${API}/api/service-records/`, { method: 'POST', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      if (!res.ok) throw new Error();
      showToast('✓ Service record saved');
      setShowForm(false); setForm(EMPTY_FORM); fetchAll(); onSaved();
    } catch { showToast('Failed to save service record', 'error'); }
      ...
      {selectedVid && (
        <>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
            <button onClick={() => setShowForm(v => !v)} style={{ ...s.primaryBtn, fontSize: 12, padding: '6px 12px', backgroundColor: showForm ? '#64748b' : '#0369a1' }}>
          ...
          {showCenterForm && (
            <div style={{ ...s.card, marginBottom: 10 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
                <div><label style={s.fieldLabel}>Name</label><input style={s.input} value={centerForm.name} onChange={e => setCenterForm(f => ({ ...f, name: e.target.value }))} placeholder="Authorized Service Center" /></div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {centers.length > 0 && centers.map(c => (
                  <span key={c.id} style={{ fontSize: 11, background: '#f1f5f9', borderRadius: 6, padding: '3px 8px', color: '#475569' }}>{c.name}</span>
                ))}
              </div>
              <button onClick={handleSaveCenter} style={{ ...s.primaryBtn, fontSize: 12, padding: '6px 12px', marginTop: 8 }}>+ Add</button>
            </div>
          )}

          {showForm && (
            <form onSubmit={handleSubmit} style={{ ...s.card, marginBottom: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 10 }}>New Service Record</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                ...
                <div>
                  <label style={s.fieldLabel}>Service Center</label>
                ...
                <div style={{ gridColumn: '1 / -1' }}><label style={s.fieldLabel}>Description</label><textarea style={{ ...s.input, height: 60 }} value={form.description} onChange={e => setF('description', e.target.value)} placeholder="Work done, issues found…" /></div>
              ...
              <div style={{ marginTop: 12 }}>
                ...
                {form.parts.map((p, i) => (
                  <div key={i} style={{ background: '#f8fafc', borderRadius: 8, padding: '8px 10px', marginBottom: 6 }}>
                    ...
                    <button type="button" onClick={() => removePart(i)} style={{ marginTop: 4, background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize: 12 }}>✕ Remove part</button>
                  </div>
                ))}
                ...
                <div key={rec.id} style={{ ...s.card, marginBottom: 8, borderLeft: `4px solid ${TYPE_COLORS[rec.service_type] || '#64748b'}` }}>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', cursor: 'pointer' }} onClick={() => setExpandId(expanded ? null : rec.id)}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: '#64748b' }}>
                        {rec.service_center_name && `${rec.service_center_name} · `}
                        {rec.odometer && `${rec.odometer?.toLocaleString('en-IN')} km`}
                        {rec.parts?.length > 0 && ` · ${rec.parts.length} part(s)`}
                      </div>
                    </div>
                  ...
                  {expanded && (
                    <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid #f1f5f9' }}>
                      ...
                      {rec.next_service_date && <div style={{ fontSize: 12, color: '#0369a1', marginBottom: 4 }}>📅 Next service: {fmtD(rec.next_service_date)}{rec.next_service_km ? ` or at ${rec.next_service_km?.toLocaleString('en-IN')} km` : ''}</div>}
                      {rec.parts?.length > 0 && (
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 8 }}>
                        <span>Labour: ₹{fmt(rec.labour_cost)}</span>
                        <span>Parts: ₹{fmt(rec.parts_cost)}</span>
                        <span style={{ fontWeight: 700 }}>Total: ₹{fmt(rec.total_cost)}</span>
                      </div>
